...
// Truncating to 60 chars made long AI-generated filenames collide, so several
// distinct photos overwrote each other on copy and repeated in the manifest.
// Keep a per-folder set and disambiguate with a counter instead.
function slugifyFile(name, taken) {
  const ext = path.extname(name).toLowerCase();
  const base = path.basename(name, path.extname(name));
  const stem = base.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60);
...
for (const [folder, slug] of Object.entries(FOLDER_TO_SLUG)) {
  ...
  for (const name of fs.readdirSync(srcDir)) {
    ...
    const outName = slugifyFile(name, taken);
